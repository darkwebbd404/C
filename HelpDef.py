...
def get_time(uid):
    url = f"http://69.62.118.127:4001/get_time/{uid}"
    res = requests.get(url)
    data = res.json()
    if "error" in data:
...
def send_likes(uid):
	likes_api_response = requests.get(f"http://69.62.118.127:5008/like?uid={uid}&key=22")
...
f"________________________")
	if likes_api_response.status_code == 200:
		...
		message = (f"________________________\n"
f" Likes Status :\n"
f" LIKES SENT !\n\n"
