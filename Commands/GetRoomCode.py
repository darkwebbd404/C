 ...
 14: ""
    ...
    def connect(self, tok, host, port, packet, key, iv):
        ...
        while True:
            ...
            #Get ROOM Id -> COMMANDE
            if "1200" in data.hex()[0:4] and b"/rmid" in data:
                room_info = re.split("/rm", str(data))[1]
                if "***" in room_info:
                    room_info = room_info.replace("***", "106")
                sid = str(room_info).split("(\\x")[0]   
                json_result = get_available_room(data.hex()[10:])
                parsed_data = json.loads(json_result)
                uid = parsed_data["5"]["data"]["1"]["data"] 
                split_data = re.split(rb'/rm', data)
                room_data = split_data[1].split(b'(')[0].decode().strip().split()
                player_id = room_data[0]
                roomname = " ".join(room_data[1:])  
                if "***" in player_id:
...
JOIN TELEGRAM CHANEL -->\n htt[c]ps:/[c]/t.[c]me/[c]pro[c]jec[c]tfo[c]xx_[c]cdx
                            """, uid
                        )
                    )
                else:
